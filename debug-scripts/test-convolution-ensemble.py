...
from ensembles.models import ConvolutionEnsemble
from ensembles.utils import EnsembleDataset


plt.figure()
losses = [
    "BinomialUnimodal_CE", "PoissonUnimodal"
]
...
for rev in revs:
    ...
    for x, y in dl:
        ...
        if do_single_plot:
            ...
            cls_ens = np.arange(1, probas.shape[1] + 1) / len(losses)
            for i in range(num_models):
                plt.plot(cls, x[0, i].cpu(), label=losses[i], ls="-", lw=1.0, alpha=0.9)
            plt.plot(cls_ens, probas[0].cpu(), label="Ensemble", ls="-", c="k", lw=2.5)
